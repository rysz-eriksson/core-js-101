...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  // eslint-disable-next-line max-len
  const degrees = (x1 * y1 + x2 * y2) / (Math.sqrt(x1 * x1 + y1 * y1) * Math.sqrt(x2 * x2 + y2 * y2));
...
function getLastDigit(value) {
  ...
  return parseInt(newString.slice(newString.length - 1), 10);
}
...
function toNumber(value, def) {
  ...
  return isFinite(result) ? result : def;
}
